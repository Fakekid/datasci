...
def load_text_data(file_name, **kwargs):
  ...
  Returns:
    if seg is not null, return a list[list[]] value, else return a list[str] value.
  ...
  with open(file_name, 'r', encoding=encoding, errors=error) as fin:
    ...
    if seg:
      content = [item.replace('\n', '').split(seg) for item in content]
    else:
      content = [item.replace('\n', '') for item in content]
  return content


def load_pandas_data(file_name, **kwargs):
  """
  ...
  data = pd.read_csv(file_name, engine=engine, seg=seg, names=names, header=header, usecols=usecols,
...
def load_np_data(file_name, **kwargs):
  ...
  data = func(file_name, delimiter=seg, dtype=dtype)
  return data
...
def load_mysql_data(sql, **kwargs):
  ...
  con = pymysql.connect(host=host, user=user, password=password, database=db, charset=charset)
  data = pd.pd.read_sql(sql=sql, con=con)
